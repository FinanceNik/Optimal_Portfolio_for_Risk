...
def dataframe_population_firstRow():
    ...
    for asset in asset_list:
        df[f'{asset}_share'][177] = df[f'{asset}_value'][177] / df[f'{asset}'][177]
    ...
    for line in range(markowitz_weights_line, 2, -6):
        try:
            for i in range(line-1, line-6, -1):
                for asset in asset_list:
                    df[f'{asset}_value'][i] = df[f'{asset}_share'][line] * df[f'{asset}'][i]

                # The portfolio value is the sum of the values of each asset class.
                df['portfolio_value'][i] = round(df['CA_value'][i] +
                                                 df['BO_value'][i] +
                                                 df['BOFC_value'][i] +
                                                 df['SE_value'][i] +
                                                 df['GE_value'][i] +
            ...
            for asset in asset_list:
                df[f'{asset}_value'][line-6] = df[f'{asset}_weight'][markowitz_weights_line] * \
                                               PortfolioRebalancingValue
            ...
            for asset in asset_list:
                df[f'{asset}_share'][line-6] = df[f'{asset}_value'][171] / df[f'{asset}'][171]

            # Calculate the portfolio value.
            df['portfolio_value'][line-6] = round(df['CA_value'][line-6] +
                                                  df['BO_value'][line-6] +
                                                  df['BOFC_value'][line-6] +
                                                  df['SE_value'][line-6] +
                                                  df['GE_value'][line-6] +
                                                  df['GES_value'][line-6] +
                                                  df['EME_value'][line-6] +
                                                  df['RE_value'][line-6], 2)
